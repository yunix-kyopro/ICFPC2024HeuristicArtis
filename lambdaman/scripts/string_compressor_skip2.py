...
# lambdaman11～16など、必ず2マスずつ移動するインスタンス専用
# 
# [使い方]
...
# サボってO(N^2)にしている
for i in range(0, len(raw_string), 2):
    s += inv_string_dict[raw_string[i]]

init_len = len(s)

print()
print(f"Initial length : {init_len * 2}")
print(f"Initial length + S : {init_len * 2 + 1}")
...
# 再帰関数の中身
result += f'L" L# L$ ? B= v$ I! S! B. B$ B$ v" B/ v# {base_base94} B- v$ I" B. BT I" BD B% v# {base_base94} S{char_list} BT I" BD B% v# {base_base94} S{char_list} '
