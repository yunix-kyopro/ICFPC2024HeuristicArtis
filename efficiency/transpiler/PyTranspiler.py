...
class PyTranspiler(ExprVisitor):
    ...
    def visit_binary_expr(self, expr: Binary):
        ...
        if expr.operator.type == TokenType.BINARY_MINUS:
            ...
            return f"{self.evaluate(expr.left)} or {self.evaluate(expr.right)}"
        elif expr.operator.type == TokenType.BINARY_AND:
            return f"{self.evaluate(expr.left)} AND {self.evaluate(expr.right)}"
        elif expr.operator.type == TokenType.BINARY_STRING_CONCAT:
            return f"{self.evaluate(expr.left)} + {self.evaluate(expr.right)}"
        elif expr.operator.type == TokenType.BINARY_STRING_TOP:
            return f"{self.evaluate(expr.right)}[:{self.evaluate(expr.left)}]"
    ...
    def visit_unary_expr(self, expr: Unary):
        ...
        if expr.operator.type == TokenType.UNARY_MINUS:
            ...
            return f"not {self.evaluate(expr.right)}"
    ...
    def visit_if_expr(self, expr: If):
        return f"{self.evaluate(expr.then_branch)} if {self.evaluate(expr.condition)} else {self.evaluate(expr.else_branch)}"
    ...
    def visit_literal_expr(self, expr: Literal):
        ...
        if expr.operator.type == TokenType.STRING:
            ...
            return f"{int(expr.operator.value)}"
    ...
    def visit_lambda_expr(self, expr: Lambda):
        ...
        # print("lambda expr")
        # return f"((v{expr.operator.value}) =>" +f"({self.evaluate(expr.function)}))" 
        return f"(lambda v{expr.operator.value}: {self.evaluate(expr.function)})"
